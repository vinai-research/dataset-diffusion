...
if __name__ == '__main__':
    ...
    prompts = [
        {
            'filename': 'test.jpg',
            'caption': 'a bicylist is riding to goal',
            'negative': 'a bicycle',
        }
    ] 
    ...
    out = pipe(batch_prompts, num_inference_steps=1, generator=generator, output_type="numpy", negative_prompt = negative_prompt)
    self_attention = None
    ...
    # clusters, out_crfs, self_attention, cross_attention = segmentor(out.images, controller, indices[:], labels[:])
        ...
        self_attention, self_attention_neg = aggregate_attention(controller, 32, ['up', 'down'], is_cross=False, select = 0)
        cross_attention, cross_attention_neg = aggregate_attention(controller, 16, ['up', 'down'], is_cross = True, select = 0)

    breakpoint() 
    image_cross = show_cross_attention(cross_attention, 16, ['up', 'down'], batch_prompts, pipe.tokenizer, select = 0)
