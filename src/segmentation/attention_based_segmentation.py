...
class Segmentor:
    ...
    def __call__(self, prompts, self_attention, *args, **kwargs):
        """
        tokenized_prompts: [list of tokens in a given sentence]
        """ 
        tokenized_prompts                    = [nltk.word_tokenize(prompt) for prompt in prompts]
        self.nouns                          = [[(i, word) for (i, (word, pos)) in enumerate(nltk.pos_tag(tokenized_prompt)) if pos[:2] == 'NN'] for tokenized_prompt in tokenized_prompts]
    ...
    def cluster2noun(self, clusters, cross_attention):
        ...
        """
        result = []
        nouns_indices = [[index for (index, word) in pair_word_index] for pair_word_index in self.nouns]
        ...
        # noun_maps_batch = np.stack(noun_maps_batch, axis = 0)
            ...
            nouns_maps = cross_attn.cpu().numpy()[:, :, [i + 1 for i in nouns_indices[idx]]]
            ...
            for c in range(self.num_segments):
                ...
                score_maps = [cluster_mask * normalized_nouns_maps[:, :, i] for i in range(len(nouns_indices[idx]))]
                scores = [score_map.sum() / cluster_mask.sum() for score_map in score_maps]
                cluster2noun_res[c] = self.nouns[np.argmax(np.array(scores))] if max(scores) > self.background_segment_threshold else "BG"
                result.append(cluster2noun_res)
        return result

    def refine_mask_classes(self, clusters, cluster2noun):
        bg_list = [idx for idx, item in enumerate(cluster2noun.values()) if isinstance(item, str) and item == 'BG'] 
        bg_pix  = bg_list[0]
        # breakpoint()
        for bg in bg_list:
            try:
