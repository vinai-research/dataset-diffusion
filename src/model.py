...
class SDMaskWrapper:
    ...
    def run_on_prompt(self, prompt: List[str], run_config):
        gen_seed = torch.Generator(self.device).manual_seed(run_config.seed)
        output = self.model(prompt, generator=gen_seed)
        image = output.images
        return image
    ...
    def register_attention_control(self):
        ...
        for name in self.model.unet.attn_processors.keys():
            cross_attention_dim = None if name.endswith(
                "attn1.processor") else self.model.unet.config.cross_attention_dim
            if name.startswith("mid_block"):
                hidden_size = self.model.unet.config.block_out_channels[-1]
                place_in_unet = "mid"
            elif name.startswith("up_blocks"):
                block_id = int(name[len("up_blocks.")])
                hidden_size = list(reversed(self.model.unet.config.block_out_channels))[
                    block_id]
                place_in_unet = "up"
    ...
    def multi_class_dense_crf(image: np.array, attention: np.array,
        ...
        h = labels.shape[0]
        w = labels.shape[1]
        U = utils.unary_from_labels(
            labels, num_classes, 0.7, zero_unsure=False)
    ...
    def generate_mask_dense_crf(self,
        ...
        for i in range(batch_size):
            for k, attention_map_list in attention_maps.items():
                # Use to store resized cross attention map (resize to the max resolution)
                list_cross_attention = []
                assert len(attention_map_list[0]) == len(
                    token_indices) == batch_size
                for attention_map in attention_map_list:
                    token_attention = attention_map[i, :, :, :, token_indices[i]].permute(
                        3, 0, 1, 2)
                    resolution = token_attention.shape[-1]

                    if resolution < max_resolution:
                        token_attention = F.interpolate(
                            token_attention, size=max_resolution, mode='bilinear')

                    list_cross_attention.append(token_attention)
                    attention_store_res[k] = torch.cat(
                        list_cross_attention, dim=1).mean(dim=1)
            ...
            average_attention_map = (average_attention_map - average_attention_map.min((1, 2), keepdims=True)) / \
                (average_attention_map.max((1, 2), keepdims=True) -
                 average_attention_map.min((1, 2), keepdims=True))
    ...
    def __call__(self,
        ...
        image = self.run_on_prompt(prompt, GEN_CONFIG)

        self_attention = self.get_attention(
            self.controller, 32, is_cross=False, prompts=prompt)
        cross_attention = self.get_attention(
            self.controller, 16, is_cross=True, prompts=prompt)
        ...
        cluster_512 = torch.nn.functional.interpolate(
            cluster, (B, image_size, image_size), mode='nearest')
        ...
        return image, cluster_512, attn_result
